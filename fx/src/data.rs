...
fn tmp_comp_opts() {
    ...
    init_tables(&conn);
    let stmt = "
        INSERT INTO posts (created, updated, content) VALUES (
            '2025-01-01 00:00:00',
            '2025-01-01 00:00:00',
            'lorem ipsum'
        );
    ";
    let mut query = conn.prepare(stmt).unwrap();
    ...
    // OKAY THIS IS GOOD ENOUGH. Lets implement via transaction.

    let stmt = "
        SELECT * FROM posts_fts WHERE posts_fts MATCH 'lor';
    ";
    let mut query = conn.prepare(stmt).unwrap();
    let opts = query
        .query_map([], |row| {
